...
class ConfigWindow(QtWidgets.QDialog):
    ...
    def __init__(self, config_name, rules, parent=None):
        ...
        self.rules = rules  # Lista de reglas (cada regla es un diccionario)
        self.resize(700, 500)
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # Botones para editar o eliminar reglas de la lista
        ...
        # Usamos QStackedWidget para cambiar los campos según la acción
        ...
        # Página para add_segment: Nuevo segmento y posición
        ...
        # Página para modify_field: Segmento, Field Index y Nuevo Valor
        ...
        # Página para reorder_fields: Segmento y Nuevo Orden (ej. "0,2,1,3")
        ...
        # Conectar el cambio de selección en el combo para actualizar la página mostrada
        self.rule_type_combo.currentIndexChanged.connect(self.stacked_widget.setCurrentIndex)

        # Botón para agregar la nueva regla
        ...
        # Botón para guardar cambios (cerrar el diálogo)
    ...
    def refresh_rules(self):
        """Actualiza la lista de reglas en la interfaz."""
        self.rules_list.clear()
        for idx, rule in enumerate(self.rules):
            self.rules_list.addItem(f"{idx+1}. {json.dumps(rule)}")

    def add_new_rule(self):
        """Agrega una nueva regla a la lista, tomando los valores de la interfaz gráfica."""
        rule_type = self.rule_type_combo.currentText()
        new_rule = {"action": rule_type}

        # Recoger los parámetros según el tipo de regla
        try:
            if rule_type == "delete_segment":
                ...
                if not (segment and field_index and new_value):
                    QtWidgets.QMessageBox.warning(self, "Error", "Todos los campos son obligatorios para modificar un campo.")
                    return
                new_rule["segment"] = segment
                ...
                if not (segment and order_text):
                    QtWidgets.QMessageBox.warning(self, "Error", "Todos los campos son obligatorios para reordenar.")
                    return
                new_rule["segment"] = segment
                # Convertir "0,2,1,3" a [0, 2, 1, 3]
                new_rule["new_order"] = [int(i.strip()) for i in order_text.split(',')]
                
            elif rule_type == "copy_value":
                segment = self.copy_segment_edit.text().strip()
                source_field = self.copy_source_field_edit.text().strip()
                dest_field = self.copy_dest_field_edit.text().strip()
                if not (segment and source_field and dest_field):
            ...
            # Limpiar los campos de la página actual para permitir ingresar otra regla fácilmente
    ...
    def edit_rule(self):
        """Permite editar la regla seleccionada mediante un diálogo de texto."""
    ...
    def get_rules(self):
        """Retorna la lista actualizada de reglas."""
        return self.rules
